...
function RunTestContent() {
  ...
  const [mode, setMode] = useState<RunMode>("join_link");
  ...
  useEffect(() => {
    ...
    if (rawMode === "test_id" || rawMode === "live_call" || rawMode === "employee" || rawMode === "join_link") {
      setMode(rawMode);
    }
    ...
    >
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <StagePanel className="space-y-5">
          <div className="flex flex-wrap gap-2">
            {(Object.keys(modeMeta) as RunMode[]).map((item) => (
              <button
                key={item}
                type="button"
                onClick={() => {
          ...
          {mode === "live_call" ? (
            <div className="space-y-3 scene-fade-up">
              ...
              {liveInvite ? <InviteCredentialsPanel invite={liveInvite} testId={liveInvite.slug?.toUpperCase() ?? null} openLabel={copy.runModes.start} /> : null}
            </div>
          ) : null}
