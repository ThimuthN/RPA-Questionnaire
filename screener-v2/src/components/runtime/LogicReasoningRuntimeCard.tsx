...
interface LogicReasoningRuntimeCardProps {
  pack: any;
  answer: Record<string, unknown>;
  onChange: (value: Record<string, unknown>) => void;
}

export function LogicReasoningRuntimeCard({ pack, answer, onChange }: LogicReasoningRuntimeCardProps) {
  const Renderer = logicReasoningDef.Renderer as any;
  const question = {
  ...
  return (
    <StagePanel className="space-y-5 border-purple-400/18 bg-[linear-gradient(180deg,rgba(147,51,234,0.12),rgba(255,255,255,0.05))]">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <StatusPill label="Logic & Reasoning" tone="purple" />
