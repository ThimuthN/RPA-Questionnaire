...
function applicationStatusTone(status: CandidateApplicationStatus): "neutral" | "blue" | "amber" | "emerald" {
...
export function CandidateSidebar({
  ...
  return (
    <aside className="flex flex-col gap-3">
      {/* Identity */}
      <div className="rounded-[22px] border border-[color:var(--app-border)] bg-[color:var(--app-surface)] p-5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <p className="font-semibold leading-tight text-[color:var(--app-heading)] truncate">{candidate.fullName}</p>
            {candidate.currentTitle ? (
              <p className="text-xs text-[color:var(--app-muted)] truncate">{candidate.currentTitle}</p>
            ) : null}
        ...
        <div className="space-y-2 border-t border-[color:var(--app-border)] pt-3 text-xs">
          ...
          {candidate.phone ? <ContactRow icon={<Phone size={12} />} value={candidate.phone} /> : null}
          {candidate.location ? <ContactRow icon={<MapPin size={12} />} value={candidate.location} /> : null}
          {linkedInHref ? (
            <a
              href={linkedInHref}
              target="_blank"
        ...
        <div className="space-y-2 border-t border-[color:var(--app-border)] pt-3">
          {candidate.roleLabel ? <MetaRow label="Role" value={candidate.roleLabel} /> : null}
          {(departmentName ?? candidate.departmentName) ? (
            <MetaRow label="Department" value={departmentName ?? candidate.departmentName!} />
          ) : null}
          {candidate.salaryExpectation ? (
            <MetaRow label="Salary exp." value={candidate.salaryExpectation} />
          ) : null}
          ...
          {candidate.batchId ? <MetaRow label="Batch" value={candidate.batchId} /> : null}
        </div>

        {/* Application */}
        {activeApplication ? (
          <div className="space-y-1 border-t border-[color:var(--app-border)] pt-3">
            <p className="text-[10px] uppercase tracking-[0.14em] text-[color:var(--app-muted)]">Application</p>
            <p className="text-xs font-medium text-[color:var(--app-heading)] truncate">{activeApplication.jobTitle}</p>
            <StatusPill
        ...
        {/* Hiring team */}
        <div className="space-y-1 border-t border-[color:var(--app-border)] pt-3">
          <p className="text-[10px] uppercase tracking-[0.14em] text-[color:var(--app-muted)]">Hiring team</p>
          {teamCount > 0 ? (
            <p className="text-xs text-[color:var(--app-text)]">{teamNames.slice(0, 3).join(", ")}{teamCount > 3 ? ` +${teamCount - 3} more` : ""}</p>
          ) : (
            <p className="text-xs text-amber-400">No team assigned</p>
          )}
        </div>
        ...
        {candidate.notesSummary ? (
          <div className="space-y-1 border-t border-[color:var(--app-border)] pt-3">
            <p className="text-[10px] uppercase tracking-[0.14em] text-[color:var(--app-muted)]">Recruiter notes</p>
            <p className="text-xs leading-5 text-[color:var(--app-text)] line-clamp-3">{candidate.notesSummary}</p>
          </div>
        ) : null}
      </div>

      {/* Actions */}
      <div className="rounded-[22px] border border-[color:var(--app-border)] bg-[color:var(--app-surface)] p-4 space-y-2">
        {candidate.stage === "applicant" && activeApplication && canPromote ? (
          <form action={`/api/candidate-applications/${activeApplication.id}`} method="post">
            ...
            <Button type="submit" className="w-full">Move to pipeline</Button>
          </form>
        ) : null}

        {canManage ? <EditCandidateInfoModal candidate={candidate} returnTo={currentDetailPath} /> : null}
