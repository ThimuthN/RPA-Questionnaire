...
function parseApplicantTab(value?: string | null): ApplicantTab {
  ...
  return applicantTabs.some((tab) => tab.key === value)
    ? (value as ApplicantTab)
    : "application";
}

function buildPlatformAssessments(
  candidate: NonNullable<Awaited<ReturnType<typeof getCandidateDetail>>>
): CandidateProfilePlatformAssessment[] {
...
function ApplicantTabs({
  ...
  function tabHref(key: ApplicantTab): Route {
    ...
    return `/people/candidates/applicants/${applicationId}${query ? `?${query}` : ""}` as Route;
  }
...
export default async function ApplicantReviewPage({
  ...
  if (!canViewApplication) redirect("/people/candidates/applicants");

  const departmentId = detail.candidate.departmentId;
  ...
  const reviewHref = `/people/candidates/applicants/${detail.id}` as Route;
  const backHref = "/people/candidates/applicants" as Route;
  ...
  const activeApplication =
    candidate.applications.find((application) => application.id === id) ??
    candidate.applications[0] ??
    null;
  const platformAssessments = buildPlatformAssessments(candidate);
  const applicationAssessments = candidate.applicationAssessments.filter(
    (application) => application.id === detail.id
  );
  const teamNames = assignments.map((assignment) => assignment.user.name ?? assignment.user.email);
  const hasAssignedPanel = assignments.length > 0;

  const candidateProfileHref = departmentId
    ? (`/people/candidates/${detail.candidate.id}?workspaceId=${departmentId}&returnTo=${encodeURIComponent(reviewHref)}` as Route)
    : (`/people/candidates/${detail.candidate.id}?returnTo=${encodeURIComponent(reviewHref)}` as Route);
    ...
    >
      ...
      <div className="flex flex-col gap-6 xl:flex-row xl:items-start">
        ...
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <ApplicantTabs applicationId={detail.id} currentTab={currentTab} />
            ...
            <div className="space-y-4">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-[color:var(--app-muted)]">
                  Review job screening responses alongside any platform assessments assigned to this applicant.
                </p>
              </div>
              <CandidateAssessmentsPanel
