  ...
  return (
    <SceneTransition>
      <PublicSiteFrame current="home" backHref={session ? "/departments" : undefined}>
          ...
          <section className="hero-card relative overflow-hidden rounded-[34px] border border-[color:var(--app-border)] px-7 py-8 shadow-[var(--app-shadow)] md:px-10 md:py-10">
            ...
            <div className="hero-grid-overlay pointer-events-none absolute inset-0 opacity-50" />
            ...
            <div className="relative z-10 grid gap-8 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-end">
              <StaggerGroup className="space-y-6" delay={0.04}>
                <StaggerItem>
                  <div className="space-y-5">
                    <div className="space-y-2">
                      <p className="text-[11px] uppercase tracking-[0.28em] text-[color:var(--pub-brand)]">Tracking trajectory</p>
                    </div>
                    <div className="max-w-4xl space-y-3">
                      <h1 className="font-display text-5xl leading-[0.94] text-[color:var(--hero-heading)] sm:text-6xl md:text-7xl">Track candidates.</h1>
                      <div className="font-display text-5xl leading-[0.94] text-[color:var(--hero-heading)] sm:text-6xl md:text-7xl" aria-hidden="true">
                        <TypedWordCycle
                          prefix=""
                          words={["Review clearly.", "Follow progress.", "Decide faster."]}
                          className="text-[color:var(--hero-heading)]"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <StaggerItem>
                  <p className="max-w-2xl text-base leading-8 text-[color:var(--hero-subtext)] sm:text-lg">
              ...
              <StaggerGroup className="grid gap-3 self-stretch sm:grid-cols-3 xl:grid-cols-1" delay={0.08}>
                {heroSignals.map((signal, index) => (
                  <StaggerItem key={signal.label}>
                    <div className="hero-signal-card h-full rounded-[22px] p-4">
                      <div className="flex items-start justify-between gap-3">
                        <p className="text-[11px] uppercase tracking-[0.22em] text-[color:var(--hero-muted)]">{signal.label}</p>
                        <span className="text-[11px] text-[color:var(--hero-muted)]">0{index + 1}</span>
                      </div>
                      <p className={`mt-4 font-display text-3xl leading-none ${signal.tone}`}>{signal.value}</p>
                      <p className="mt-3 text-sm leading-6 text-[color:var(--hero-subtext)]">{signal.detail}</p>
                    </div>
                  </StaggerItem>
                ))}
              </StaggerGroup>
