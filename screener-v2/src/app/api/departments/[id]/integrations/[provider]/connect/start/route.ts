...
import { startDepartmentProviderConnection } from "@/lib/integrations/service";
...
export async function POST(
  ...
  try {
    ...
    const result = await startDepartmentProviderConnection(id, provider);
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return jsonError("Unknown provider.", 404);
    }
    return jsonError(error instanceof Error ? error.message : "Could not start integration connection.");
  }
}
