...
import { setProviderAppEnabled } from "@/lib/integrations/service";
...
export async function POST(
  ...
  try {
    ...
    await setProviderAppEnabled(provider, false);
    return NextResponse.json({ ok: true });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return jsonError("Unknown provider.", 404);
    }
    return jsonError(error instanceof Error ? error.message : "Could not disable provider.");
  }
}
